...
const FeatureCard = ({ children }: { children: React.ReactNode }) => {
  return (
    <div
      className={`transition ease-in-out delay-100 duration-300 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-900 hover:to-indigo-900 text-black 
      hover:text-white fill-resume-800 px-6 py-10 lg:p-12 flex shadow-md cursor-pointer relative rounded-xl h-full`}
    >
...
const CardPinnnedIcon = ({ children }: { children: React.ReactNode }) => {
  return (
    <div className="backdrop-blur-2xl bg-purple-300 rounded-full p-2 shadow-level-hard absolute right-0 top-0 -mt-4 -mr-1">
      {children}
    </div>
  );
};
